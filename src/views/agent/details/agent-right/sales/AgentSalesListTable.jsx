...
const AgentSalesListTable = ({ dataList }) => {
  // States
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('status', {
        ...
        cell: ({ row }) => (
          <Chip
            ...
            color={row?.original?.status === 'Achived' ? 'success' : 'error'}
            size='small'
          />
        )
      }),
      columnHelper.accessor('stockIn', {
        header: 'Stock In',
        cell: ({ row }) => <Typography>{row.original.stockIn}</Typography>
      }),
      columnHelper.accessor('stockOut', {
        header: 'Stock Out',
        cell: ({ row }) => <Typography>{row.original.stockOut}</Typography>
      }),
      columnHelper.accessor('stockRemaining', {
        header: 'Balance',
        cell: ({ row }) => {
          return row.original.stockRemaining === 0 ? (
            <Chip variant='tonal' label='Paid' color='success' size='small' />
          ) : (
            <Typography color='text.primary'>{row.original.stockRemaining}</Typography>
          )
        }
      })
    ],
...
export default AgentSalesListTable
